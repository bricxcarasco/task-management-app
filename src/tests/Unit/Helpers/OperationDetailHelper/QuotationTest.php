...
class QuotationTest extends TestCase
{
     ...
     * @return void
     */
    public function testTestSuccessQuotationRegistration()
    {
        // Fetch Quotation form
        $form = Form::where('type', Types::QUOTATION)->first();
     ...
     * @return void
     */
    public function testTestFailedQuotationRegistration()
    {
        // Fetch Quotation form
        $form = Form::where('type', Types::QUOTATION)->first();
     ...
     * @return void
     */
    public function testTestSuccessQuotationEditBasicInformation()
    {
        // Fetch Quotation form
        $form = Form::where('type', Types::QUOTATION)->first();
        ...
        foreach ($inputData as $field => $data) {
            if ($form->$field != $data) {
                $field = OperationDetailHelper::quotationFormFields()[$field];
                $expected .= __('Operation - Edit Basic Information', ['field' => $field]) . "\r\n";
            }
        }

        // Actual output
        $actual = OperationDetailHelper::getEditMessage($form, $inputData, Types::QUOTATION);
