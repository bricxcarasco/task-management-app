...
class CreateTableGroupConnectiionUsers extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('group_connectiion_users', function (Blueprint $table) {
            $table->id()->comment('id for Laravel');
            $table->unsignedBigInteger('group_connection_id');
            $table->unsignedBigInteger('rio_id');
            $table->integer('status')->comment('0:申請中 、1:つながり状態（否認の場合はdeleted_atをnot nullに設定）');
            ...
            $table->foreign('group_connection_id')->references('id')->on('group_connections');
    ...
    {
        Schema::dropIfExists('group_connectiion_users');
    }
}
