...
<div class="container position-relative zindex-5 pt-6 py-md-6 mb-md-3 home--height">
    ...
    <div class="row">
        ...
        <div class="d-flex flex-column pb-4 pb-md-0 rounded-3 ">
            ...
            <div>
            <a class="btn btn-link" href="{{ route('classifieds.sales.edit', $productId) }}"><i class="ai-arrow-left me-2"></i>{{ __('Return to the Input Screen') }}</a>
            </div>
            <h5 class="text-center">{{ __('Product Information') }}</h5>
        </div>
        <form method="POST" action="{{ route('classifieds.sales.edit-complete', $productId) }}" novalidate>
            @csrf
            @method('POST')
            <div class="mb-3" @if (empty($product['upload_file']) && empty($product['local_file'])) class="d-none" @endif>
                ...
                <div class="file-uploader file-uploader--grid col-12 d-none">
                    <input type="file" class="js-product-file-uploader"
                ...
                <div class="js-product-file-uploader-local-files d-none">
                    @foreach ($product['local_file'] ?? [] as $path)
                        @if (!empty($path))
                ...
                {{-- No uploaded image/s --}}
                @if (empty($product['upload_file'] ?? []) && empty($product['local_file'] ?? []))
