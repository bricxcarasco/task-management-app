        ...
        <div class="row">
            <div class="col-12 col-md-9 offset-md-3">
                <div class="d-flex align-items-center justify-content-between border-bottom">
                    <a href="{{ route('forms.invoices.index') }}" class="btn btn-link">
                        <i class="ai-arrow-left"></i>
                    </a>
                    <div class="d-flex align-items-center justify-content-around">
                        <button type="button" class="btn btn-link" data-bs-toggle="modal"
                            data-bs-target="#form-document-linkage">{{ __('Send to Document Management') }}</button>
                        <a href="{{ route('forms.invoices.update-history', $form->id) }}"
                            class="btn btn-link p-0 me-3">{{ __('History2') }}</a>
                        <a href="{{ route('forms.invoices.edit', $form->id) }}"
                            class="btn btn-link p-0 me-3">{{ __('Edit') }}</a>
                        <button type="button" class="btn btn-link js-delete-form"
                            data-action="{{ route('forms.invoices.destroy', ['form' => $form->id, 'withAlert' => true]) }}">
                            {{ __('Delete') }}
                        </button>
                    </div>
                </div>
                ...
                {{-- Payment Notes --}}
                @if (!empty($form->issuer_payment_notes))
                    <p class="mb-0 bg-dark-gray p-2 c-white">{{ __('Payment Notes') }}</p>
                    <div class="p-4">
                        <ul class="list-style-none">
                            <li>{!! nl2br(e($form->issuer_payment_notes)) ?? '' !!}</li>
                        </ul>
                    </div>
                @endif

                {{-- Payee Information --}}
                @if (!empty($form->issuer_payee_information))
                    <p class="mb-0 bg-dark-gray p-2 c-white">{{ __('Payee Information') }}</p>
                    <div class="p-4">
                        <ul class="list-style-none">
                            <li>{!! nl2br(e($form->issuer_payee_information)) ?? '' !!}</li>
                        </ul>
                    </div>
                @endif
