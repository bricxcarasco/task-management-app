        ...
        <form action="" @submit="submitForm" ref="formRef" novalidate>
          ...
          <div class="modal-body">
            <div class="row">
              ...
              <div class="col-12">
                <!-- Skill Input Field -->
                <label class="form-label" for="registratin-skill-field">
                  {{ $t('labels.skill') }}
  ...
  setup(props, { emit }) {
    ...
    const updateModalTitle = () => {
      modalTitle.value = isEdit()
        ? i18n.global.t('headers.skill_edit')
        : i18n.global.t('headers.skill_registration');
    };

    /**
     * Event listener for add skill form submit
     *
     * @returns {void}
     */
    const submitForm = async (event) => {
      ...
      const apiCall = isEdit()
        ? neoProfileApiService.updateSkill(formData.value.id, formData.value)
        : neoProfileApiService.addSkill(neo.value.id, formData.value);
