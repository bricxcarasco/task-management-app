          ...
          <div class="modal-body">
            <div class="mb-3">
              <label for="text-input" class="form-label">{{
                $t('labels.free_word_search_for_delivery_slips')
              }}</label>
            ...
            <div class="mb-3">
              ...
              <div class="d-flex align-items-center justify-content-around">
                <div>
                  <input
                    v-model="formData.delivery_deadline_start_date"
                ...
                <div>
                  <input
                    v-model="formData.delivery_deadline_end_date"
                    class="form-control datepicker2_ymd_end rounded"
                    :class="
                      errors?.delivery_deadline_end_date != null
                        ? 'is-invalid'
                        : ''
                    "
                    type="text"
                  ...
                  <base-validation-error
                    :errors="errors?.delivery_deadline_end_date"
                  />
                </div>
              </div>
            </div>
            ...
            <div class="text-center mt-4">
              <button
                ...
                @click="handleSearchDeliverySlips"
              >
                {{ $t('buttons.search') }}
              </button>
            </div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
     ...
     */
    const resetModal = () => {
      const modal = document.querySelector('#delivery-search-modal');
     ...
     * @returns {void}
     */
    const handleSearchDeliverySlips = async () => {
      ...
      await deliverySlipApiService
        .validateSearchInputs(data)
        .then((response) => {
          resetModal();
          emit('search-deliveries', response.data.data);
        })
        .catch((error) => {
