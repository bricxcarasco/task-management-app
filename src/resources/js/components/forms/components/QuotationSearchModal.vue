          ...
          <div class="modal-body">
            <div class="mb-3">
              <label for="text-input" class="form-label">{{
                $t('labels.free_word_search_for_quotations')
              }}</label>
            ...
            <div class="mb-3">
              <label>{{ $t('labels.specify_expiration_date') }}</label>
              <div class="d-flex align-items-center justify-content-around">
                <div>
                  <input
                    v-model="formData.expiration_start_date"
                ...
                <div>
                  <input
                    v-model="formData.expiration_end_date"
                    class="form-control datepicker2_ymd_end rounded"
                    :class="
                      errors?.expiration_end_date != null ? 'is-invalid' : ''
                    "
                    type="text"
                  />
                  <base-validation-error
                    :errors="errors?.expiration_end_date"
                  />
                </div>
              </div>
            </div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
     ...
     */
    const resetModal = () => {
      const modal = document.querySelector('#quotation-search-modal');
    ...
    const handleSearchQuotations = async () => {
      ...
      await quotationApiService
        ...
        .then((response) => {
          resetModal();
          emit('search-quotations', response.data.data);
        })
        .catch((error) => {
