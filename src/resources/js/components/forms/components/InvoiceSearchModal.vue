          ...
          <div class="modal-body">
            <div class="mb-3">
              <label for="text-input" class="form-label">
                {{ $t('labels.free_word_search_for_invoices') }}
              </label>
            ...
            <div class="mb-3">
              <label>{{ $t('labels.specify_payment_date') }}</label>
              <div class="d-flex align-items-center justify-content-around">
                <div>
                  <input
                    v-model="formData.payment_start_date"
                ...
                <div>
                  <input
                    v-model="formData.payment_end_date"
                    class="form-control datepicker2_ymd_end rounded"
                    :class="
                      errors?.payment_end_date != null ? 'is-invalid' : ''
                    "
                    type="text"
                  />
                  <base-validation-error :errors="errors?.payment_end_date" />
                </div>
              </div>
            </div>
            <div class="mb-3">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
     ...
     */
    const resetModal = () => {
      const modal = document.querySelector('#invoice-search-modal');
    ...
    const handleSearchInvoices = async () => {
      ...
      await invoiceApiService
        ...
        .then((response) => {
          resetModal();
          emit('search-invoices', response.data.data);
        })
        .catch((error) => {
