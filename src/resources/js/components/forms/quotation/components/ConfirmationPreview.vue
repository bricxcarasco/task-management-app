        ...
        <div class="col-12 col-md-9 offset-md-3">
          ...
          >
            <a
              href="#"
              class="btn btn-link"
              @click="() => $emit('switch-quotation-create-form', form_data)"
            >
...
export default defineComponent({
  name: 'QuotationConfirmationPreview',
  props: {
    form_data: {
      type: [Array, Object],
      required: true,
  ...
  setup(props, { emit }) {
    ...
    const logoDisplayTemp = computed(
      () => `/api/forms/file/restore?code=${props.form_data.logo}`
    );
    ...
    const logoDisplay = computed(() => {
      if (props.is_edit || props.is_duplicate) {
        const issuerImage = props.form_data.issuer_image;

        if (
          issuerImage &&
          !props.form_data.logo &&
        ...
        if (issuerImage && !props.form_data.logo) {
        ...
        return props.form_data.logo
          ? logoDisplayTemp.value
          : `/hero-storage/public/forms/issuer/profile_image/${issuerImage}`;
      }

      return props.form_data.logo ? logoDisplayTemp.value : defaultImage.value;
    });
    ...
    const handleSaveQuotation = () => {
      ...
      formData.value.price = parseInt(productPriceWithTaxes.value.total, 10);

      if (props.is_duplicate) {
        emit('save-quotation-duplicate', formData.value);
        return;
      }

      if (props.is_edit) {
        emit('save-quotation', formData.value);
        return;
      }

      if (!props.is_duplicate && !props.is_edit) {
        // Handle responses
        formsApiService
